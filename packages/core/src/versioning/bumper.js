 ...
 */
export function bumpVersion(ctx, options) {
 ...
 */
export function generateDiffSummary(before, after) {
    const parts = [];
    // key_files
    ...
    if (addedKf > 0)
        parts.push(`+${addedKf} key_files`);
    if (modifiedKf > 0)
        parts.push(`~${modifiedKf} key_files`);
    if (removedKf > 0)
        parts.push(`-${removedKf} key_files`);
    // contracts
    ...
    if (addedC > 0)
        parts.push(`+${addedC} contracts`);
    if (removedC > 0)
        parts.push(`-${removedC} contracts`);
    // decisions
    ...
    if (addedD > 0)
        parts.push(`+${addedD} decisions`);
    if (removedD > 0)
        parts.push(`-${removedD} decisions`);
    // gotchas
    const gotchaDiff = after.gotchas.length - before.gotchas.length;
    if (gotchaDiff > 0)
        parts.push(`+${gotchaDiff} gotchas`);
    if (gotchaDiff < 0)
        parts.push(`${gotchaDiff} gotchas`);
    // tags
    ...
    if (addedT > 0)
        parts.push(`+${addedT} tags`);
    if (removedT > 0)
        parts.push(`-${removedT} tags`);
    // summary
